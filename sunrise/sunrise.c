 ...
 * @return int Return code
 */
static inline int sr_write_meter_control(sensor_config_t* config);
...
int sunrise_read_config(sensor_config_t* config)
{
    ...
    uint8_t buf[11] = {0};
    uint8_t data = 0;
    if ((ret = sr_read(REG_MEAS_MODE, buf, 11)) != 0) return ret; // Read measurement registers
...
static int sr_write_config(sensor_config_t* config)
{
    ...
    *((uint16_t*)&command_buf[9]) = ntoh16(config->abc_target_value);
    ...
    if ((ret = sr_write(REG_MEAS_MODE, command_buf, 11)) != 0) return ret; // Write measurement registers
    return SUCCESS;
}
...
static inline int sr_write_meter_control(sensor_config_t* config)
{
    ...
    uint8_t buf;
    int32_t ret;
    if ((ret = sr_read(REG_METER_CONTROL, &buf, 1)) != 0) return ret; // Read meter control register
