...
int32_t cozir_lp3_write_config(cozir_lp3_config_t* config)
{
    ...
    if (config->alarm_en)
    {
        *((uint16_t*)&buf[0]) = ntoh(config->alarm_treshold_co2);
        if ((ret = cozir_lp3_write(REG_ALARM_LEVEL, buf, 2)) != 0) return ret;
    }
    else
    {
        *((uint16_t*)&buf[0]) = ntoh(0);
        if ((ret = cozir_lp3_write(REG_ALARM_LEVEL, buf, 2)) != 0) return ret;
    }
}
